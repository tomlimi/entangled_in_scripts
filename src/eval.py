...
from studentBert import TeacherStudentBert
...
def eval_single_model(args):
    ...
    if args.model_type == "student":
        model = TeacherStudentBert.from_pretrained(model_dir_path, teachers=None, masks=None).student_model
    elif args.model_type == "teacher":
        model = AutoModelForMaskedLM.from_pretrained(model_dir_path)
    else:
        raise ValueError(f"Wrong model type: {args.model_type}, supported: 'student' 'teacher'")
...
if __name__ == '__main__':
    ...
    parser.add_argument('-mt', '--model_type', type=str, required=True, help="Pick from 'teacher' 'student'")

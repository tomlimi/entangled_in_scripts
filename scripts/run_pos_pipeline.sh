...
do
  ...
  jid=$(sbatch scripts/finetune_pos.sh $alpha $train_alpha $vocab_size $src_lang $tok_type $seed)
  echo ${jid##* }
  echo $src_lang
  echo $seed
  echo $alpha
  ...
  do
    ...
    #sbatch --killable --requeue scripts/eval_pos.sh $alpha $train_alpha $vocab_size $src_lang $tgt_lang $seed
    sbatch --dependency=afterany:${jid##* } scripts/eval_pos.sh $alpha $train_alpha $vocab_size $src_lang $tgt_lang $tok_type $seed
  done
done
